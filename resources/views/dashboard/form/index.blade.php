      ...
      <table class="w-full text-sm text-left text-gray-700 dark:text-gray-200">
        <thead class="text-xs uppercase bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-300">
          <tr>
            ...
            <th class="px-6 py-3">Jawaban & Nilai</th>
        ...
        <tbody>
          @foreach ($gejalas as $gejala)
            <tr class="border-b border-gray-200 dark:border-slate-700">
              <td class="px-6 py-4 font-medium">{{ $gejala->kode }}</td>
              <td class="px-6 py-4">{{ $gejala->gejala }}</td>
              <td class="px-6 py-4">
              </td>
              <td class="px-6 py-4 flex space-x-3">
                ...
                <form action="{{ route('gejala.destroy', $gejala->id) }}" method="POST"
                  ...
                  @method('DELETE')
                  <button type="submit"
                    class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-xl text-sm px-4 py-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                    Hapus
                  </button>
                </form>
              </td>
