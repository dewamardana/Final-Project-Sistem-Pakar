...
@section('content')
  <div class="bg-white dark:bg-slate-800 shadow rounded-lg p-6 mx-auto max-w-2xl">
    <h2 class="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-100">Edit Bobot Penilaian</h2>

    <form action="{{ route('bobot_penilaian.update', $bobot->id) }}" method="POST">
      @csrf
      @method('PUT')
      ...
      <div class="mb-4">
        ...
        <input type="text" name="certainty_term" id="certainty_term"
          value="{{ old('certainty_term', $bobot->certainty_term) }}"
      ...
      <div class="mb-4">
        ...
        <input type="number" step="0.01" name="cf" id="cf" value="{{ old('cf', $bobot->cf) }}"
          class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring
      ...
      {{-- Actions --}}
      <button type="submit"
        class="text-white bg-yellow-400 hover:bg-yellow-500 focus:outline-none focus:ring-4 focus:ring-yellow-300 font-medium rounded-xl text-sm px-5 py-2.5">
        Update
      </button>
