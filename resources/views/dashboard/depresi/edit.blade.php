...
@section('content')
  <div class="bg-white dark:bg-slate-800 shadow rounded-lg p-6 max-w-2xl mx-auto">
    <h2 class="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-100">Edit Tingkat Depresi</h2>
    <form action="{{ route('depresi.update', $depresi->id) }}" method="POST" class="space-y-4">
      @csrf
      @method('PUT')
      <div>
        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-200">Kode</label>
        <input type="text" name="kode" value="{{ old('kode', $depresi->kode) }}"
      ...
      <div>
        ...
        <input type="text" name="depresi" value="{{ old('depresi', $depresi->depresi) }}"
          class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring">
        @error('depresi')
          <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
        @enderror
